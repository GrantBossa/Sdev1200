...
500      self.property_tax_value = tk.StringVar()
      ...
      self.my_entry = tk.Entry(
         self.entry_frame, bd=5, textvariable=self.text_entry_variable
      )
      ...
      self.calculate_button = tk.Button(
         self.button_frame, text="Calculate Values", command=self.calculateValues
      )
      ...
      self.property_label = tk.Label(
         self.info_frame, textvariable=self.property_value
      )
      self.assessment = tk.Label(self.info_frame, textvariable=self.assessment_value)
      self.property_tax_label = tk.Label(
         self.info_frame, textvariable=self.property_tax_value
      )
   ...
   def calculateValues(self):
      self.float_property_value = float(self.my_entry.get())
      self.float_assessment_value = float(self.float_property_value) * 0.6
      self.float_property_tax_value = float(self.float_assessment_value) * 0.0075

      self.property_value.set(
         f"{"Property Value":<24} $ {self.float_property_value:>10.2f}"
      )
      self.assessment_value.set(
         f"{"Assessment Value":<20} $ {self.float_property_value * 0.6:>10.2f}"
      )
      self.property_tax_value.set(
         f"{"Property Tax":<25} $ {self.float_assessment_value * .0075:>10.2f}"
      )
